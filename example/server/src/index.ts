...
EP2KeyBIP.create(
  ...
  TEST_VALUES.testConfig.server.seed

)
  .then((key) => {
    app.use(cors());

    app.use(ExpressEP2PushServer(
      key,
      { keys: TEST_VAPID_KEYS, subject: TEST_VALUES.testConfig.vapid.subject },
      server,
      { path: TEST_VALUES.testConfig.server.EP2_PUSH_CTX }
    ));

    app.use(ExpressEP2PeerServer(key, server, {
      key: "ep2peer",

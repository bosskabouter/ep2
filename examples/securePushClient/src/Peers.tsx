...
function Peers (): JSX.Element {
  ...
  return (
    <div><div>
       <input checked={local} type='checkbox' onChange={(ev) => { setLocal(ev.target.checked) }}/> {local ? 'local secure server' : 'public peer server'}
       </div>
...
function PeerInstance ({
  ...
  const [securePeer, setSecurePeer] = useState<OnlineClient>()
  ...
  function shortenBase64 (base64Value: string | undefined): string {
    if (base64Value === undefined || base64Value === null) return 'no id'
    const maxLength = 10
  ...
  function getColorFromBase64 (base64Value: string | undefined): string {
    if (base64Value === undefined || base64Value === null) return 'red'
    let hash = 0

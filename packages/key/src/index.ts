...
export class EP2Key {
   ...
   * 
   * @param publicKey 
   * @returns undefined if not a hex value
   */
  static convertPeerId2PublicKey(publicKey: string): Uint8Array | undefined {
    if (/^[0-9a-fA-F]+$/.test(publicKey))
      console.info('not a hex:' + publicKey)
    return /^[0-9a-fA-F]+$/.test(publicKey) ? sodium.from_hex(publicKey) : undefined;
  }

 ...
 */
export class OnlineClient extends Peer {
  /**
   ...
   */
  constructor (private readonly key: EP2Key, public readonly serverPublicKey?: string, options?: PeerJSOption) {
    super(key.peerId, (serverPublicKey === undefined || serverPublicKey === null)
      ? options
      : options = { ...options, token: JSON.stringify(key.initiateHandshake(serverPublicKey).handshake) }
    )
    super.on('open', (id) => { this.handleOpenServer(id) })
 ...
 */
  ...
  public async isServerSecure (): Promise<boolean> {
    const insecurePeer = new Peer(`${Math.round(Math.random() * 1000000000)}`, {
      ...this.options, debug: 0, logFunction (_logLevel, ..._rest) { }
    })

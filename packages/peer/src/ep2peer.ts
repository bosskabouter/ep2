...
export class EP2Peer extends Peer {
  ...
  constructor(
    ...
    super(
      key.peerId,
      serverPublicKey === undefined || serverPublicKey === null
        ? options
        : (options = {
          ...options,
    ...
    this.isEp2PeerServer = (serverPublicKey) ? this.testEp2Server() : Promise.resolve(false)
  ...
  private async testEp2Server(): Promise<boolean> {
    const insecurePeer = new Peer(`${Math.round(Math.random() * 100000000000)}`, {
      ...this.options,
      debug: 0,

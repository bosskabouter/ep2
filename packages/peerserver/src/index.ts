...
function initialize(
  ...
  server.on("connection", (client: IClient) => {
    try {
      ep2key.receiveHandshake(client.getId(), JSON.parse(client.getToken()));
    } catch (error: any) {
      const msg = `Invalid Handshake from client[${client.getId()}, reason: ${error.toString()}] `
      console.warn(msg)
      client.getSocket()?.close();
      server.emit("invalid-handshake: ", msg);
    }
  });
  return server;
}
